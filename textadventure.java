...
public class textadventure {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        while (!cont) {
            if (affirmative(kb)) {
                if (inventory.contains("Pants")) {//they got the pants
                    cont = true;
                    turns_survived++;
        ...
        if (inventory.contains("Pants")) {//they got more pants
        ...
        } else {//no mo pants (only had 1 pair)
    ...
    public static void pause(int x) {
        try {
            ...
            System.err.println("An unexpected error occurred while sleeping!");
            e.printStackTrace();
            System.exit(1);
        }//BAD PROGRAMMING, FTW!
    }
    ...
    public static void lsinv(String inventory) {
        if (inventory != null) {
            ...
            for (String anInv : inv) out(anInv);
        }
        if (inventory != null && inventory.equals("")) out("You have no items.\n");
    }
